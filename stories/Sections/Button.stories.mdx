...
Support Hub comes with predefined set of colors for buttons. The color of the button can be set with `color-scheme` to `woot-button`.
...
There are diffrent styles that comes with Support Hub buttons. This can set with `variants` prop for `woot-button`. By deault the filled will be the style, other variations are `smooth, hollow, clear, link`
...
Support Hub currently use [IonIcons version 2](https://ionic.io/ionicons/v2) which are font-icons. The button position is fixed on the left.
